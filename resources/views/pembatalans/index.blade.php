<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <div class="flex justify-end mb-4">
            <a href="{{ route('cetak-pembatalan') }}" target="_blank"
                ...
                Cetak Data Laporan Pembatalan
            </a>
            <a href="{{ route('pembatalans.create') }}"
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Tanggal Pembatalan</th>
                            <th class="px-4 py-3">Action</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y">
                        @foreach ($pembatalan as $batal)
                            <tr class="text-gray-700">
                                <td class="px-4 py-3 text-sm">
                                    {{ $batal->peminjaman->ruangan->nama_ruangan }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $batal->peminjaman->peminjam->name }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $batal->peminjaman->tanggal_pinjam }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $batal->tanggal_pembatalan }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex space-x-2 items-center">
                                        <a href="{{ route('pembatalans.show', $batal->id) }}"
                                            class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                                            Show
                                        </a>
                                        <a href="{{ route('pembatalans.edit', $batal->id) }}"
                                                    ...
                                                    <div class="mt-4 mb-6">
                                                        ...
                                                        <p class="text-sm text-gray-700">
                                                            Apakah Anda yakin ingin menghapus data <span
                                                                class="font-semibold">{{ $batal->nama_peminjam }}</span>?
                                                        </p>
                                                    </div>
                                                    <footer
                                                        ...
                                                        <form action="{{ route('pembatalans.destroy', $batal->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
